...
t1 <- tbl_regression(glmer(data = daily3, Total_Steps ~ Pre.Post + gender + Age + rate_health + (1|ID))) |> add_glance_source_note() %>%
  modify_header(label = "**Adjusted Model**") 

t2<- tbl_regression(glmer(data = daily3, Total_MVPA ~ Pre.Post + gender + Age + rate_health + (1|ID))) |> add_glance_source_note() %>%
  modify_header(label = "**Adjusted Model**") 

t3 <- tbl_regression(glmer(data = daily3, Total_Sleep ~ Pre.Post + gender + Age + rate_health + (1|ID))) |> add_glance_source_note() %>%
  modify_header(label = "**Adjusted Model**") 
...
# Diff-in-diff Plot
plot_data <- daily2[complete.cases(daily3$gender), ] %>%
  ...
  summarize(Avg_Steps = mean(Total_Steps, na.rm=T),
            se = sd(Total_Steps, na.rm=T) / sqrt(n()),
            upper = Avg_Steps + (1.96 * se),
            lower = Avg_Steps + (-1.96 * se))

plot_data <- labelled::copy_labels(from = daily3, to = plot_data)
  plot_data <- subset(plot_data, !is.na(gender))
...
plot_data <- daily2[complete.cases(daily3$gender), ] %>%
  ...
  mutate(gender = factor(gender, labels = c("Female", "Male")),
         Pre.Post = factor(Pre.Post, labels = c("Before DCI", "After DCI"))) %>%
  group_by(gender, Pre.Post) %>%
  summarize(Avg_MVPA = mean(Total_MVPA, na.rm=T),
            se = sd(Total_MVPA, na.rm=T) / sqrt(n()),
            upper = Avg_MVPA + (1.96 * se),
            lower = Avg_MVPA + (-1.96 * se))

plot_data <- labelled::copy_labels(from = daily3, to = plot_data)
  plot_data <- subset(plot_data, !is.na(gender))

# Plot
ggplot(plot_data, aes(x = Pre.Post, y = Avg_MVPA, color = gender)) +
  geom_pointrange(aes(ymin = lower, ymax = upper), size = 1) +
  geom_line(aes(group = gender)) +
  theme_bw() + 
...
plot_data <- daily2[complete.cases(daily3$gender), ] %>%
  ...
  mutate(gender = factor(gender, labels = c("Female", "Male")),
         Pre.Post = factor(Pre.Post, labels = c("Before DCI", "After DCI"))) %>%
  group_by(gender, Pre.Post) %>%
  summarize(Avg_Sleep = mean(Total_Sleep, na.rm=T),
            se = sd(Total_Sleep, na.rm=T) / sqrt(n()),
            upper = Avg_Sleep + (1.96 * se),
            lower = Avg_Sleep + (-1.96 * se))

plot_data <- labelled::copy_labels(from = daily3, to = plot_data)
  plot_data <- subset(plot_data, !is.na(gender))

# Plot
ggplot(plot_data, aes(x = Pre.Post, y = Avg_Sleep, color = gender)) +
  geom_pointrange(aes(ymin = lower, ymax = upper), size = 1) +
  geom_line(aes(group = gender)) +
  theme_bw() + 
  labs(title="Interaction of Sex and Time", y = "Average Daily Sleep", x = "")
